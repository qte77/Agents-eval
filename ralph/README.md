...
- ~~**Intermediate progress visibility**~~: **DONE** — Monitor now tails agent log output at 30s intervals with `[CC]` (magenta) prefix for agent activity and red for agent errors, alongside existing phase detection from git log.
  - **CC monitor log nesting** (**TODO**): `monitor_story_progress` (`ralph.sh:199`) reads `tail -5 "$LOG_FILE"` — the same file it writes to via `log_cc` (line 206). Each 30s cycle re-reads its own wrapped output, producing `[CC] [INFO] [CC] [INFO] ...` chains. The `sed` strip (line 201) only removes the outer `[INFO]` prefix, not nested `[CC]` prefixes. Fix: track byte offset (`wc -c`) between cycles, read only new bytes with `tail -c +$offset`.

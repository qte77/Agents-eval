## Best Practices

- [MAS Design Principles](${BLOB}/docs/best-practices/mas-design-principles.md): Multi-agent system design guidance
- [MAS Security](${BLOB}/docs/best-practices/mas-security.md): Security practices for agent systems
- [Testing Strategy](${BLOB}/docs/best-practices/testing-strategy.md): Testing approach and guidelines
- [Python Best Practices](${BLOB}/docs/best-practices/python-best-practices.md): Python coding standards

## Optional

- [Security Advisories](${BLOB}/docs/security-advisories.md): Known security issues and mitigations
- [Agent Instructions](${BLOB}/AGENTS.md): AI agent behavioral rules and compliance requirements
- [PeerRead Agent Usage](${BLOB}/docs/howtos/peerread-agent-usage.md): Operational guide for the evaluation pipeline
- [CC Adoption Plan](${BLOB}/docs/analysis/ClaudeCode/CC-adoption-plan.md): Claude Code feature adoption roadmap
- [Agent Teams Orchestration](${BLOB}/docs/analysis/ClaudeCode/CC-agent-teams-orchestration.md): Claude Code agent teams analysis
- [Ralph Enhancement Research](${BLOB}/docs/analysis/ClaudeCode/CC-ralph-enhancement-research.md): Ralph loop gap analysis and external patterns
- [Skills Adoption](${BLOB}/docs/analysis/ClaudeCode/CC-skills-adoption-analysis.md): Claude Code skills system adoption analysis
- [Sandboxing](${BLOB}/docs/analysis/ClaudeCode/CC-sandboxing-analysis.md): Claude Code sandboxing and security analysis
- [Model & Provider Configuration](${BLOB}/docs/analysis/ClaudeCode/CC-model-provider-configuration.md): LLM model and provider configuration
- [Plugin Packaging](${BLOB}/docs/analysis/ClaudeCode/CC-plugin-packaging-research.md): Claude Code plugin packaging research
- [Memory System](${BLOB}/docs/analysis/ClaudeCode/CC-memory-system-analysis.md): Claude Code memory system analysis
- [Cowork & Plugins Enterprise](${BLOB}/docs/analysis/ClaudeCode/CC-cowork-plugins-enterprise-analysis.md): Enterprise Cowork and plugins analysis
- [Changelog Feature Scan](${BLOB}/docs/analysis/ClaudeCode/CC-changelog-feature-scan.md): Claude Code changelog feature tracking
- [Remote Control](${BLOB}/docs/analysis/ClaudeCode/CC-remote-control-analysis.md): Claude Code remote control analysis
- [Cloud Sessions](${BLOB}/docs/analysis/ClaudeCode/CC-cloud-sessions-analysis.md): Claude Code cloud sessions (Code on the Web)
- [Fast Mode](${BLOB}/docs/analysis/ClaudeCode/CC-fast-mode-analysis.md): Claude Code fast mode analysis
- [Chrome Extension](${BLOB}/docs/analysis/ClaudeCode/CC-chrome-extension-analysis.md): Claude Code Chrome extension analysis
- [llms.txt & Platform Docs](${BLOB}/docs/analysis/ClaudeCode/CC-llms-txt-analysis.md): llms.txt spec, Anthropic docs index, and project implementation
- [Remote Access Landscape](${BLOB}/docs/analysis/ClaudeCode/CC-remote-access-landscape.md): Remote access landscape analysis
- [Inference Providers](${BLOB}/docs/analysis/Inference-Providers.md): LLM provider comparison
- [Landscape: Agent Frameworks](${BLOB}/docs/landscape/landscape-agent-frameworks-infrastructure.md): Agent framework survey
- [Landscape: Evaluation Resources](${BLOB}/docs/landscape/landscape-evaluation-data-resources.md): Evaluation datasets and benchmarks
- [Landscape: Research Agents](${BLOB}/docs/landscape/landscape-research-agents.md): Research agent landscape

...
class Suggestion(BaseModel):
    ...
    metric: str = Field(description="Metric name that triggered this suggestion (e.g., 'cosine_score')")
    tier: int = Field(ge=1, le=3, description="Evaluation tier (1=Traditional, 2=LLM Judge, 3=Graph)")
    severity: SuggestionSeverity = Field(description="Severity level: critical, warning, or info")
    message: str = Field(description="Human-readable description of the issue referencing the metric and score")
